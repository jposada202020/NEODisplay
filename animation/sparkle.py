...
class Sparkle(Animation):
    ...
    def __init__(
        ...
        if len(self._mask) >= len(pixel_object):
            raise ValueError(
                "Sparkle mask should be smaller than number pixel array"
            )
    ...
    def draw(self):
        self._pixels = [
            self._random_in_mask() for _ in range(self._num_sparkles)
        ]
    ...
    def after_draw(self):
        ...
        for pixel in self._pixels:
            ...
            if (pixel + 1) % self._num_pixels in self._mask:
                self.pixel_object[(pixel + 1) % self._num_pixels] = (
                    self._dim_color
                )
